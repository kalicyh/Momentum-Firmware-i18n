...
static const uint8_t
    enclave_signature_expected_clipper[ENCLAVE_FACTORY_KEY_SLOTS][ENCLAVE_SIGNATURE_SIZE] = {
        {0xE8, 0xE8, 0xC5, 0x62, 0x64, 0x11, 0x63, 0x58, 0x35, 0x32, 0x77, 0x31, 0x9E, 0x73, 0xE4, 0x7D},
        {0x7E, 0x61, 0xFB, 0x2D, 0x36, 0xF1, 0x56, 0x10, 0xFD, 0x59, 0xBD, 0x8A, 0xBB, 0x59, 0x20, 0x42},
        {0x68, 0xD4, 0xA5, 0x78, 0x4A, 0x68, 0xBF, 0x14, 0x32, 0xB8, 0x49, 0x03, 0x4A, 0x28, 0xA8, 0x20},
        {0x92, 0x24, 0x3D, 0x41, 0x6C, 0x76, 0x90, 0xF5, 0xB9, 0xB7, 0x27, 0x46, 0x0F, 0x93, 0x2F, 0x41},
        {0xB6, 0x64, 0xCF, 0xA1, 0x04, 0xEF, 0x0B, 0x2A, 0x4F, 0xE5, 0xFB, 0xFF, 0xB4, 0x7B, 0xAE, 0xFC},
        {0x73, 0xF5, 0x8D, 0x9F, 0x12, 0x3D, 0xB5, 0x84, 0x61, 0x7C, 0x71, 0x21, 0x57, 0x50, 0x5F, 0x0F},
...
bool furi_hal_crypto_enclave_verify(uint8_t* keys_nb, uint8_t* valid_keys_nb) {
    ...
    const uint8_t(*expected)[ENCLAVE_SIGNATURE_SIZE] = enclave_signature_expected;
#ifdef MOMENTUM_DEVICE_CLIPPER
        expected = enclave_signature_expected_clipper;
#endif
    uint8_t keys = 0;
    ...
    for(size_t key_slot = 0; key_slot < ENCLAVE_FACTORY_KEY_SLOTS; key_slot++) {
        if(furi_hal_crypto_enclave_load_key(key_slot + 1, enclave_signature_iv[key_slot])) {
            ...
            if(furi_hal_crypto_encrypt(
                ...
                keys_valid += memcmp(buffer, expected[key_slot], ENCLAVE_SIGNATURE_SIZE) == 0;
            }
