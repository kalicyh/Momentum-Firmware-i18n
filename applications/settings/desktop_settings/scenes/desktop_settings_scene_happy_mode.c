...
void desktop_settings_scene_happy_mode_on_enter(void* context) {
    ...
    dialog_ex_set_header(
        app->dialog_ex, DESKTOP_SETTINGS_UI_TEXT("Happy Mode", "快乐模式"), 64, 0, AlignCenter, AlignTop);
    dialog_ex_set_text(
        app->dialog_ex,
    ...
    dialog_ex_set_left_button_text(
        app->dialog_ex,
        settings.happy_mode ? DESKTOP_SETTINGS_UI_TEXT("Disable", "关闭") :
                              DESKTOP_SETTINGS_UI_TEXT("Go back", "返回"));
    dialog_ex_set_right_button_text(
        app->dialog_ex,
        settings.happy_mode ? DESKTOP_SETTINGS_UI_TEXT("Keep enabled", "保持启用") :
                              DESKTOP_SETTINGS_UI_TEXT("Enable", "启用"));
