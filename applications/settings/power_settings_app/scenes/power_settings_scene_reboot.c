...
void power_settings_scene_reboot_on_enter(void* context) {
    ...
    submenu_set_header(submenu, POWER_SETTINGS_UI_TEXT("Reboot Type", "重启方式"));
    submenu_add_item(
        submenu,
        POWER_SETTINGS_UI_TEXT("Firmware Upgrade", "固件升级"),
        PowerSettingsRebootSubmenuIndexDfu,
        power_settings_scene_reboot_submenu_callback,
        app);
    submenu_add_item(
        submenu,
        POWER_SETTINGS_UI_TEXT("Reboot Flipper", "重启设备"),
        PowerSettingsRebootSubmenuIndexOs,
        power_settings_scene_reboot_submenu_callback,
        app);
