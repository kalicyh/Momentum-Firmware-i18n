...
const char* const bt_settings_text[BtSettingNum] = {
    BT_SETTINGS_UI_TEXT("OFF", "关"),
    BT_SETTINGS_UI_TEXT("ON", "开"),
};
...
void bt_settings_scene_start_on_enter(void* context) {
    ...
    if(furi_hal_bt_is_gatt_gap_supported()) {
        item = variable_item_list_add(
            var_item_list,
            BT_SETTINGS_UI_TEXT("Bluetooth", "蓝牙"),
            BtSettingNum,
        ...
        variable_item_list_add(
            var_item_list, BT_SETTINGS_UI_TEXT("Unpair All Devices", "取消全部配对"), 1, NULL, NULL);
        variable_item_list_set_enter_callback(
            var_item_list, bt_settings_scene_start_var_list_enter_callback, app);
    } else {
        item = variable_item_list_add(
            var_item_list, BT_SETTINGS_UI_TEXT("Bluetooth", "蓝牙"), 1, NULL, NULL);
        variable_item_set_current_value_text(item, BT_SETTINGS_UI_TEXT("Broken", "异常"));
    }
