...
void bt_settings_scene_forget_dev_confirm_on_enter(void* context) {
    ...
    dialog_ex_set_header(
        dialog, BT_SETTINGS_UI_TEXT("Unpair All Devices?", "取消全部配对？"), 64, 0, AlignCenter, AlignTop);
    dialog_ex_set_text(
        dialog,
        BT_SETTINGS_UI_TEXT("All previous pairings\nwill be lost!", "之前的所有配对\n都会丢失！"),
        64,
        14,
        AlignCenter,
        AlignTop);
    dialog_ex_set_left_button_text(dialog, BT_SETTINGS_UI_TEXT("Cancel", "取消"));
    dialog_ex_set_right_button_text(dialog, BT_SETTINGS_UI_TEXT("Unpair", "解除"));
