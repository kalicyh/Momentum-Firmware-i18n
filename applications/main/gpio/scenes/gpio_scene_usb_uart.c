...
bool gpio_scene_usb_uart_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        dialog_ex_set_left_button_text(dialog, GPIO_UI_TEXT("Exit", "退出"));
        dialog_ex_set_right_button_text(dialog, GPIO_UI_TEXT("Stay", "停留"));
        dialog_ex_set_header(
            dialog, GPIO_UI_TEXT("Exit USB-UART?", "退出 USB-UART?"), 22, 12, AlignLeft, AlignTop);
