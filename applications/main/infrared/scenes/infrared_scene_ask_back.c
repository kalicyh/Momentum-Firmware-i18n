...
void infrared_scene_ask_back_on_enter(void* context) {
    ...
    if(infrared->app_state.is_learning_new_remote) {
        dialog_ex_set_header(
            dialog_ex,
            INFRARED_UI_TEXT("Exit to Infrared Menu?", "退出到红外菜单?"),
            64,
            11,
            AlignCenter,
            AlignTop);
        ...
        dialog_ex_set_header(
            dialog_ex,
            INFRARED_UI_TEXT("Exit to Remote Menu?", "退出到遥控菜单?"),
            64,
            11,
            AlignCenter,
            AlignTop);
    ...
    dialog_ex_set_text(
        dialog_ex,
        INFRARED_UI_TEXT("All unsaved data\nwill be lost!", "所有未保存数据\n都会丢失!"),
        64,
        25,
        AlignCenter,
        AlignTop);
    dialog_ex_set_icon(dialog_ex, 0, 0, NULL);
    dialog_ex_set_left_button_text(dialog_ex, INFRARED_UI_TEXT("Exit", "退出"));
    dialog_ex_set_center_button_text(dialog_ex, NULL);
    dialog_ex_set_right_button_text(dialog_ex, INFRARED_UI_TEXT("Stay", "停留"));
