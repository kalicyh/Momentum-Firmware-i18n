...
void ibutton_scene_start_on_enter(void* context) {
    ...
    submenu_add_item(
        submenu, IBUTTON_UI_TEXT("Read", "读取"), SubmenuIndexRead, ibutton_submenu_callback, ibutton);
    submenu_add_item(
        submenu, IBUTTON_UI_TEXT("Saved", "已保存"), SubmenuIndexSaved, ibutton_submenu_callback, ibutton);
    submenu_add_item(
        submenu,
        IBUTTON_UI_TEXT("Add Manually", "手动添加"),
        SubmenuIndexAdd,
