...
void momentum_app_scene_protocols_freqs_on_enter(void* context) {
    ...
    item = variable_item_list_add(
        var_item_list,
        MOMENTUM_UI_TEXT("Use Defaults", "使用默认"),
        2,
    ...
    variable_item_set_current_value_text(
        item, app->subghz_use_defaults ? MOMENTUM_UI_TEXT("ON", "开") : MOMENTUM_UI_TEXT("OFF", "关"));

    item = variable_item_list_add(var_item_list, MOMENTUM_UI_TEXT("Static Freqs", "静态频率"), 0, NULL, app);
    variable_item_set_current_value_text(item, ">");

    item = variable_item_list_add(var_item_list, MOMENTUM_UI_TEXT("Hopper Freqs", "跳频频率"), 0, NULL, app);
    variable_item_set_current_value_text(item, ">");
